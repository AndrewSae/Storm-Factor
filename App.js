...
import { Keyboard,FlatList, ActivityIndicator, Image, StyleSheet, Text, View, ImageBackground, SafeAreaView, TextInput, TouchableOpacity, ScrollView} from 'react-native';
...
export default function App() {
  ...
  async function onPress() {
    ...
    if (city === "") {
      ...
      try{
        const x = await getCords(city);
        const data = await getWeather(x[0], x[1])
...
useEffect(() => {
  (async () => {
    ...
    if (status !== 'granted') {
      ...
      const data = await getWeather(cords[0], cords[1])
      await setWeather(data)
      await sethourly(await data["hourly"])
      await setDaily(await data["daily"])
      console.log(daily[5])
    ...
    {/* title */}
      <View style={styles.titleContainer}>
        ...
        <View style={styles.searchContainer}>
          {/* city inpit */}
        <TextInput  style={errorStatus ? styles.errorInput : styles.input} placeholder="Enter a city" onSubmitEditing={onPress} value={city} onChangeText={text => setCity(text)}/>
        {/* search button */}
...
{/*----------------------------------------------------------Future Weather---------------------------------------------------------- */}
    ...
    <View style={styles.furtureContainer}>
        <FlatList 
            ...
            renderItem={({ item }) => (
              <FurtureForecast temp={item["temp"].day} icon={item["weather"][0].icon} dec={item["weather"][0].description} dt={item["dt"]}/>
              )}/>
      </View>
...
{/*----------------------------------------------------------Other Data Section---------------------------------------------------------- */}
      ...
      <View style={styles.titleContainer}>
        ...
        <View style={styles.moreDataContainer}>
            ...
            {weather.current["wind_gust"] ? <OtherDataCard icon={<Feather name="wind" size={50} color={colors.iconColor} />} text={"Gusts"} data={Math.round(weather.current["wind_gust"])+" mph"}/> : null}
